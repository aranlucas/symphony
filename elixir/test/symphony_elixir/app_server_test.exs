defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server treats top-level tool/requestUserInput like item-scoped tool input" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-top-level-tool-user-input-#{System.unique_integer([:positive])}"
      )

    try do
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "MT-718A")
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-top-level-tool-user-input.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-top-level-tool-user-input.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
        ...
        case \"$count\" in
          ...
          2)
            ;;
          3)
            printf '%s\\n' '{\"id\":2,\"result\":{\"thread\":{\"id\":\"thread-718a\"}}}'
            ;;
          4)
            printf '%s\\n' '{\"id\":3,\"result\":{\"turn\":{\"id\":\"turn-718a\"}}}'
            printf '%s\\n' '{\"id\":113,\"method\":\"tool/requestUserInput\",\"params\":{\"itemId\":\"call-718a\",\"questions\":[{\"header\":\"Provide context\",\"id\":\"freeform-718a\",\"isOther\":false,\"isSecret\":false,\"options\":null,\"question\":\"What comment should I post back to the issue?\"}],\"threadId\":\"thread-718a\",\"turnId\":\"turn-718a\"}}'
            ;;
          5)
            printf '%s\\n' '{\"method\":\"turn/completed\"}'
            exit 0
      ...
      issue = %Issue{
        id: "issue-top-level-tool-user-input",
        identifier: "MT-718A",
        title: "Top level tool input answer",
        description: "Ensure top-level tool input prompts receive a generic answer",
        state: "In Progress",
        url: "https://example.org/issues/MT-718A",
        labels: ["backend"]
      }
      ...
      assert {:ok, _result} =
               AppServer.run(workspace, "Handle top-level tool input", issue, on_message: on_message)
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 payload["id"] == 113 and
                   get_in(payload, ["result", "answers", "freeform-718a", "answers"]) == [
